import pymongo, logging, os, logger
...
class MongoDB(object):
    ...
    def __iter__(self):
        for doc in self.retrieve_many_records(self.collection, {}, no_cursor_timeout = False):
            yield record

    @property
    def database(self):
        """Obtain and set the database used. Setter accepts s sting"""
        return self.db
    ...
    @property
    def collection(self):
        """Obtain and set the collection used. Setter accepts s sting"""
        return self.coll
    ...
    def retrieve_many_records(self, collection, query, **kwargs):
        """Returns all records that satisfy query
                ...
                for doc in self.conn.retrieve_many_records(coll, {}, no_cursor_timeout = False):
                    if 'Posts' in coll:
                        if db == 'VoatDB':
                            ...
                            tokenised = word_tokenize(doc['selftext'].lower() + ' ' + doc['title'].lower())
                    else:
                        if db == 'VoatDB':
